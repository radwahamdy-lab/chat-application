...
ChatServer::ChatServer(boost::asio::io_context &io, int port) : acceptor(io, tcp::endpoint(tcp::v4(), port)) {
    startAccept(io);
}

void ChatServer::startAccept(boost::asio::io_context &io) {
    ...
    acceptor.async_accept(
        ...
        [this, client, &io](boost::system::error_code ec) {
            ...
            startAccept(io); // similar to while loop
        });
}

void ChatServer::registerClient(string usrname, ClientSession &client) {
    clients.push_back(&client);
...
void ChatServer::handleMsg(ClientSession &client, Msg msg) {
    switch(msg.type) {
        case MsgType::LOGIN:{
            ...
            {
                Msg error{MsgType::ERR, "server", msg.sender, "User cannot be found"};
                client.send(error);
            }
            break;
        }
        case MsgType::REGISTER:{
            users.createUser(msg.sender, msg.content);
            break;
        }
        case MsgType::CHAT:{
            ...
            {
                Msg error{MsgType::ERR, "server", msg.sender, "Receiver cannot be found"};
                client.send(error);
            }
            break;
        }
